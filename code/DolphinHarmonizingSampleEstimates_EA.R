...
e <- a %>% 
  ...
  #filter(preservation != "longmires") %>% # remove samples preserved in later because protocol was not quite right 
...
data_small <- e %>% 
  select(reality_idx, treatment_idx, pore_size_um, preservation, extraction, logVol) %>% 
  distinct() %>% 
