...
## NOW RATIO!!! 
v_ps_tt <- v_ps_tt %>% 
  mutate(DNA_total_copies_ul = (DNA_total_conc*6.022E23)/(80*618*10E9)) %>% 
  mutate(ratio_tt_total = (Tt_total_conc/DNA_total_copies_ul))

## target is 80 bp - use NEB https://nebiocalculator.neb.com/#!/dsdnaamt
## 1.132e+10 copies of target = 1 ng of target --- OLD
### instead use general 816 g/mol for average bp 
...
#### try plotting target on y and total on x
ggplot(v_ps_tt, aes(x=DNA_total_conc, y=Tt_total_conc, color=filter_size)) + 
  geom_point() + 
  theme_bw() + 
  facet_wrap(~vol_filtered, scales="free") + 
  labs(y="Target DNA (copies/L)", x = "Total DNA (ng/ul)", color="Pore Size (um)")
```
...
p_e <- p_e_raw %>% 
  filter(filter_v_liq == "filter") %>% # only keep samples used filter 
  filter(smithroot_v_vaccum == "SR") %>% # only keep samples filtered on site 
  filter(inhibited==0) %>% # for now, only keep things not inhibited 
  mutate(quantity=as.numeric(quantity)) %>% 
  mutate(quantity2=dilution*quantity) %>% 
  mutate(total_DNA=as.numeric(total_DNA)) %>% 
  mutate(DNA_total_copies_ul = (total_DNA*6.022E23)/(80*618*10E9)) %>% 
  mutate(ratio_tt_total = (quantity2/DNA_total_copies_ul)) %>% 
...
p1 <-  ggplot(p_e, aes(x=preservation, y=total_DNA, fill=extraction)) + 
  geom_boxplot() + 
  theme_bw() + 
  labs(y="ng/uL", x = " ", title = "Total DNA",  fill="Extraction Method")

p2 <-  ggplot(p_e, aes(x=preservation, y=log10(quantity2), fill=extraction)) + 
  geom_boxplot() + 
  theme_bw() + 
  labs(y="Log10 copies/uL", x = " ", title = "Target DNA", fill="Extraction Method") 

p3 <-  ggplot(p_e, aes(x=preservation, y=ratio_tt_total, fill=extraction)) + 
  geom_boxplot() + 
  theme_bw() + 
...
ggsave(here("3panel.png"),width=6,height=8, units="in")
...
compare_homogenized <- v_ps %>% 
  filter(filter_size == "5um") %>% 
  filter(vol_filtered == "3L") %>% 
  filter(use==1) %>% 
  summarize(n=n(), meanconc = mean(Tt_total_conc), sdconc=sd(Tt_total_conc)) %>% 
  mutate(cv = sdconc/meanconc)
...
ggplot(bio_var_stats, aes(x=preservation, y=cv, shape=extraction, color=extraction)) + 
  geom_point(position=position_dodge(0.5), size=4) +
  # geom_errorbar(aes(ymin=meanconc-sdconc, ymax=meanconc+sdconc), width=.2,
  #                position=position_dodge(0.5)) + 
  theme_bw() + 
  geom_hline(yintercept=compare_homogenized$cv, linetype="longdash") + 
  labs(x="Preservation Method", y="Coefficient of Variation", color="Extraction Method") 
...
v_ps_merge <- v_ps_tt %>% 
  select(c(lab_ID, rep, filter_size, vol_filtered, DNA_total_conc, Tt_total_conc, ratio_tt_total)) %>% 
  mutate(preservation="longmires") %>% 
  mutate(extraction="PCI") %>% 
  mutate(experiment="v_ps") %>% 
  mutate(homogenized = 1) %>% 
  rename(sample=lab_ID, tech_rep=rep) %>% 
  mutate(filter_size=as.numeric(str_extract(filter_size, "[0-9]+"))) %>% 
  mutate(vol_filtered=as.numeric(str_extract(vol_filtered, "[0-9]+")))

p_e_merge <- p_e %>% 
  select(c(sample, tech_rep, preservation, extraction, total_DNA, quantity2, ratio_tt_total)) %>% 
  mutate(filter_size=5) %>% 
  mutate(vol_filtered=3) %>% 
  mutate(experiment="p_e") %>% 
  mutate(homogenized = 0) %>% 
  rename(DNA_total_conc=total_DNA, Tt_total_conc=quantity2)

experiments_merged2 <- rbind(v_ps_merge, p_e_merge)

write_rds(experiments_merged2, file=here("data","experiments_merged.RDS"))
```
